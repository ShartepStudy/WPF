...
namespace WCF_DuplexSvc
{
    ...
    {
        /*
         * метод ReturnTime() каждый раз запускается в новом потоке, 
         * для запуска потока используется делегат ParameterizedThreadStart, 
         * способный передать один параметер типа object, 
         * но поскольку нам надо передать в поток два параметра типа int, 
         * мы упаковываем их в List<int> и в таком виде передаем методу потока SendTimeToCLient() 
         */
    ...
    /*
     * метод SendTimeToCLient() будет вызван после обращения клиента к нашей службе. 
     * Он подождет до начала следующей минуты и начнет отправлять клиенту сообщения с 
     * периодичностью period миллисекунд, всего таких сообщений он отправит number. 
     * Каждое из этих сообщений доставляется клиенту методом ReceiveTime() 
     */
